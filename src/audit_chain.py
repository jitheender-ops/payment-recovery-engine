...
folding in the one before it. Anyone can then independently recompute the
whole chain from the raw rows and prove nothing in it has been altered —
that's verify_chain(), and it does not trust the stored hashes at all, only
recomputes and compares.

Run on demand via `python -m src.audit_chain --stamp` / `--verify`, or from
any async context via the two functions directly. Not wired into the
...
def _chain_hash(prev_hash: str, row: CaseEvent) -> str:
    return hashlib.sha256((prev_hash + _canonical_repr(row)).encode()).hexdigest()
...
async def stamp_unhashed_events(session: AsyncSession, *, batch_size: int = 5000) -> int:
    ...
    while True:
        ...
        for row in rows:
            ...
            row.event_hash = _chain_hash(running_hash, row)
...
async def verify_chain(session: AsyncSession) -> ChainVerification:
    ...
    """
    rows = (
        await session.execute(select_all_events_ordered())
    ).scalars().all()
    ...
    for row in rows:
        if row.event_hash is None:
            return ChainVerification(
                intact=False, events_checked=len(rows), first_broken_id=row.id,
                detail=f"event {row.id} was never stamped — run stamp_unhashed_events first",
            )
        expected = _chain_hash(running_hash, row)
    ...
    return ChainVerification(
        intact=True, events_checked=len(rows), first_broken_id=None,
        detail=f"{len(rows)} events verified, chain intact",
    )
