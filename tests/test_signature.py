...
def test_valid_signature_passes(webhook_secret: str, signed_payload: tuple[bytes, str]) -> None:
    raw_body, signature = signed_payload
    assert verify_webhook_signature(raw_body, signature, webhook_secret) is True


def test_invalid_signature_rejected(webhook_secret: str, signed_payload: tuple[bytes, str]) -> None:
    raw_body, _ = signed_payload
    assert verify_webhook_signature(raw_body, "invalid_sig", webhook_secret) is False


def test_empty_signature_rejected(webhook_secret: str, signed_payload: tuple[bytes, str]) -> None:
    raw_body, _ = signed_payload
    assert verify_webhook_signature(raw_body, "", webhook_secret) is False


def test_modified_body_rejected(webhook_secret: str, signed_payload: tuple[bytes, str]) -> None:
...
def test_different_secret_rejected(signed_payload: tuple[bytes, str]) -> None:
    raw_body, signature = signed_payload
    assert verify_webhook_signature(raw_body, signature, "wrong_secret") is False


def test_unicode_body_handling() -> None:
    body = '{"name": "राजपे टेस्ट"}'.encode()
    secret = "test_secret"
