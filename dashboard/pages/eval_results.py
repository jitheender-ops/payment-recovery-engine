...
for policy, metrics in results.items():
    rows.append({
        "Policy": policy,
        "Recovery Rate (%)": f"{metrics.get('recovery_rate_%', 0):.1f} ± {metrics.get('recovery_rate_%_std', 0):.1f}",
...
with col1:
    ...
    fig = go.Figure(go.Bar(x=policies, y=recovery_vals, error_y=dict(type="data", array=recovery_err),
...
with col2:
    ...
    fig2 = go.Figure(go.Bar(x=policies, y=rev_vals, error_y=dict(type="data", array=rev_err),
                            marker_color=["#e74c3c", "#f39c12", "#3498db", "#2ecc71"][:len(policies)]))
...
if "XGBoost/Rules" in results:
    ...
    st.success(f"**Headline: ₹{xgb_rev:,.0f} recovered per ₹1Cr of failed volume at {xgb_rate:.1f}% recovery rate**")
